...
def optimize_hyperparams(cfg, pipeline):
    ...
    param_space = dict()
    for key in cfg["hyperparam_opt"]:
        if (key != 'hyperparam_opt') and (isinstance(cfg["hyperparam_opt"][key], Iterable)) and (
...
def sklearn_gridsearch(hparam_config, cv, param_space, estimator):
    """
    Performs a grid search over the parameter space for the given estimator and cross validation.
    :param estimator: sklearn estimator
    :param cv: sklearn cross validation
    :param parameter_space: dictionary with the parameters to be tested
