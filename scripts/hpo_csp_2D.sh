...
for SUBJECT in "${SUBJECTS[@]}"; do
  ...
  apptainer run -B /tmp/${SUBJECT}.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_env.sif python ./src/baseline_train.py tune=True experiment=0_csp_2D hparams_search=hpo_csp_MNE +data.subject_sessions_dict="{$SUBJECT: "all"}" logger.mlflow.experiment_name="HPO-CSP-2D" logger.mlflow.run_name="${SUBJECT}-2D-CSP" paths.results_dir="tune_results"
