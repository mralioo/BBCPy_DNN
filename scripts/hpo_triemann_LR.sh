...
for SUBJECT in "${SUBJECTS[@]}"; do
  ...
  apptainer run -B /tmp/${SUBJECT}.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_env.sif python ./src/baseline_train.py tune=True experiment=0_riemann_tangent_LR hparams_search=hpo_0_riemann_tangent +data.subject_sessions_dict="{$SUBJECT: "all"}" logger.mlflow.experiment_name="HPO-TRiemann-LR" logger.mlflow.run_name="${SUBJECT}-LR-TRiemann"
