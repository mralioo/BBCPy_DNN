...
for SUBJECT in "${SUBJECTS[@]}"; do
    ...
    apptainer run --nv -B /tmp/${SUBJECT}.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_env.sif python ./src/dnn_train.py experiment=2_Tsception_LR +data.subject_sessions_dict="{$SUBJECT: "all"}" logger.mlflow.experiment_name="Tsception-LR" logger.mlflow.run_name="${SUBJECT}-LR"
