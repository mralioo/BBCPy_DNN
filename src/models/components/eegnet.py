...
from src.models.components.modules import Expression, Ensure4d
...
class EEGNetv4(nn.Sequential):
    ...
    def __init__(
            ...
            drop_prob=0.25, trial=None
    ):
        super().__init__()

        if isinstance(trial, optuna.Trial):
            self.F1 = trial.suggest_int("F1", low=6, high=10, step=2)
            self.D = trial.suggest_int("D", low=2, high=4, step=2)
            self.F2 = trial.suggest_int("F2", low=12, high=16, step=2)
...
class EEGNetv1(nn.Module):
    ...
    def __init__(
        ...
        self.model.add_module("elu_2", Expression(elu))
        self.model.add_module("pool_2", pool_class(kernel_size=(2, 4), stride=(2, 4)))
        self.model.add_module("drop_2", nn.Dropout(p=self.drop_prob))
        ...
        self.model.add_module("elu_3", Expression(elu))
        self.model.add_module("pool_3", pool_class(kernel_size=(2, 4), stride=(2, 4)))
        self.model.add_module("drop_3", nn.Dropout(p=self.drop_prob))
