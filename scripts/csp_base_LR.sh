#!/bin/bash
#SBATCH --job-name=csp_base-LR
#SBATCH --partition=cpu-2h
#SBATCH --gpus-per-node=0
#SBATCH --ntasks-per-node=8
#SBATCH --output=../jobs_outputs/csp_base-LR/%x_%j.o
#SBATCH --error=../jobs_outputs/csp_base-LR/%x_%j.e
...
for SUBJECT in "${SUBJECTS[@]}"; do
  ...
  apptainer run -B /tmp/${SUBJECT}.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_env.sif python ./src/baseline_train.py experiment=0_csp_base_LR +data.subject_sessions_dict="{$SUBJECT: "all"}" logger.mlflow.experiment_name="CSP-base-LR" logger.mlflow.run_name="${SUBJECT}-LR-CSP-base"
