...
for experiment in "${experiments[@]}"; do
    ...
    apptainer run -B /tmp/S3.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_lightning_v3.sif python ./src/baseline_train.py +experiment=${experiment} +data.subject_sessions_dict="{S3: "all"}" logger.mlflow.experiment_name="A-RL-S3-all"
