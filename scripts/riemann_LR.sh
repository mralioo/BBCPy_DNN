...
for SUBJECT in "${SUBJECTS[@]}"; do
  ...
  apptainer run -B /tmp/${SUBJECT}.sqfs:/input-data:image-src=/ ./../env_images/bbcpy_env.sif python ./src/baseline_train.py experiment=0_riemann_LR +data.subject_sessions_dict="{$SUBJECT: "all"}" logger.mlflow.experiment_name="RIEMANN-LR" logger.mlflow.run_name="${SUBJECT}-LR-RIEMANN"
